...
# TIMEFRAME
predictionslist=$C_predictionslist
...
# grib filename final destination
destination="gfs-forecast.grb"
# stack results flag
stackresults="0"
...
# keep safe previous created full GRIB file
if test -f $workingfolder/$destination; then
        echo "INFO: Saving previous found resulting GRIB file in .old file"
	mv -f $workingfolder/$destination $workingfolder/$destination.old
	if [[ $stackresults = "1" ]]; then 
                echo "INFO: Copying stack file"
		cp $workingfolder/$destination.old $workingfolder/$destination
	fi
fi
# merge downloaded files into the full resulting file and remove temporal folder
echo "INFO: Creating one GRIB file"
if [[ $stackresults = "1" ]]; then 
	cat $workingfolder/$tmpfolder/*.grb >> $workingfolder/$destination
else
	cat $workingfolder/$tmpfolder/*.grb > $workingfolder/$destination
fi
